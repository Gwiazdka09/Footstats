...
## KRYTYCZNE PROBLEMY (wykryte 2026-05-24)

| Problem | Severity | Plik |
|---------|----------|------|
| `response_cache.py` uciety — brak eviction functions | 🔴 KRYTYCZNY | `core/response_cache.py` (linia 255 urwana) |
| NULL bytes na koncu pliku | 🔴 KRYTYCZNY | `ai/analyzer.py` (31 null bytes), `core/async_utils.py` (90 null bytes) |
| `pyproject.toml` ma dead deps (psycopg2 w main deps zamiast optional) | 🟠 Medium | `pyproject.toml` |
| `_RAM_CACHE` brak eviction/max size | 🟠 Medium | `utils/cache.py` |
| 209x `except Exception` (bare) | 🟠 Medium | top: sts.py(16), superbet.py(15), base_playwright.py(14), daily_agent.py(13) |
| `gui/node_modules/` = 3.1 GB w repo | 🟠 Medium | `src/footstats/gui/node_modules/` |
| `poisson.py` unused imports (6 symbolow) | 🟡 Low | `core/poisson.py` |
| 39 uncommitted dirty files | 🟡 Low | `git status` |
...
- **P4.4** Cache cleanup ✅
- **P4.5** Root cleanup ✅
- **P4.6** Partial imports cleanup ✅ (częściowo — poisson.py, classifier.py do zrobienia)
...
| **Disk** | ⚠️ Bloat | gui/node_modules 3.1GB, 12 starych PDF, aider cache |
| **Git** | ⚠️ Dirty | 39 uncommitted changes |
...
**NATYCHMIAST (P0):**
1. Naprawic response_cache.py (przywrocic z git lub dopisac brakujace funkcje)
2. Usunac null bytes z analyzer.py i async_utils.py
3. Commit dirty files lub git stash

**NASTEPNA SESJA (P1):**
1. P4.3 Exception handling — top 5 plikow
2. Dodac eviction do `_RAM_CACHE` w cache.py
3. Wyrzucic gui/node_modules do .gitignore, usunac z repo
