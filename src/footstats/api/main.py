...
from footstats.api.routes.terminarz import router as terminarz_router
...
def _init_db() -> None:
    """Tabele bazowe. DDL siedzi w `footstats.db.schema` — jedyne zrodlo.

    Wczesniej ta funkcja miala wlasna kopie schematu, a `core/backtest.init_db()`
    druga, ROZNA (patrz naglowek `db/schema.py`). Obie pisaly do tej samej bazy.
    """
...
app.include_router(terminarz_router)
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8000)  # nosec B104 — kontener (Cloud Run) wymaga 0.0.0.0
