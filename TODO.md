...
- [x] **Kontuzje v2 — baza graczy + goal_share** (07-05): `core/player_db.py` (SQLite) + `scrapers/player_stats.py`. Wpięte w `_apply_injury_corrections` → kara napastnika ∝ udziałowi w golach. Populacja przez **`scripts/refresh_players.py`** (`--season`, `--only`, `--understat`).
  ...
  - **Do wpięcia (payoff):** (1) λ kadr → predykcje mundialu (zamiast Bzzoiro-ML) w quick/weekly_picks; (2) rating 1-10 → scoring/lineup_strength (jakość gracza/składu). Oba teraz tylko zapisane. Denominator goal_share kadr = top-50 strzelców (kraje z 1 strzelcem → 100%, bounded cap).
